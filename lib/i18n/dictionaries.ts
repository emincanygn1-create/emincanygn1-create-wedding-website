import type { Locale } from "./config";

const tr = {
  nav: {
    home: "Ana Sayfa",
    couple: "Çift",
    event: "Detaylar",
    gallery: "Galeri",
    rsvp: "Katılım",
    wishes: "Dilekler",
    moments: "Anılar",
    story: "Hikayemiz",
    faq: "S.S.S.",
    gift: "Hediye",
  },
  cover: {
    eyebrow: "Davetlisiniz",
    scroll: "Aşağı Kaydırın",
    theWeddingOf: "Düğün Davetiyesi",
    dear: "Sayın",
    guest: "Değerli Misafirimiz",
    nameNote: "İsim veya unvan yazımında bir hata olduysa affınıza sığınırız.",
    open: "Davetiyeyi Aç",
  },
  couple: {
    eyebrow: "Bizi Tanıyın",
    title: "Mutlu Çift",
    bride: "Gelin",
    groom: "Damat",
    photoPlaceholder: "Fotoğraf Alanı",
  },
  countdown: {
    eyebrow: "Geriye Kalan Süre",
    title: "Büyük Güne Sayılı Günler",
    days: "Gün",
    hours: "Saat",
    minutes: "Dakika",
    seconds: "Saniye",
    saveDate: "Tarihi Kaydedin",
    addToCalendar: "Takvime Ekle",
  },
  events: {
    eyebrow: "Ne Zaman, Nerede",
    title: "Etkinlik Detayları",
    ceremony: "Nikah Töreni",
    reception: "Düğün Resepsiyonu",
    combined: "Nikah ve Düğün",
    map: "Haritada Gör",
  },
  gallery: {
    eyebrow: "Anılarımız",
    title: "Fotoğraf Galerisi",
    empty: "Fotoğraflar yakında eklenecek.",
  },
  video: {
    eyebrow: "Bizim Hikayemiz",
    title: "Video",
  },
  rsvp: {
    eyebrow: "Lütfen Bize Bildirin",
    title: "Katılım Formu",
    subtitle:
      "Yerinizi ayırabilmemiz için katılım durumunuzu paylaşır mısınız?",
    name: "Adınız Soyadınız",
    email: "E-posta (isteğe bağlı)",
    phone: "Telefon (isteğe bağlı)",
    attendingQuestion: "Katılabilecek misiniz?",
    yes: "Evet, orada olacağım",
    no: "Maalesef katılamıyorum",
    guestCount: "Kaç kişi geleceksiniz?",
    side: "Kimin davetlisisiniz?",
    sideBride: "Gelin",
    sideGroom: "Damat",
    sideBoth: "İkisi de",
    diet: "Yemek tercihi veya alerji (isteğe bağlı)",
    dietHint: "Vejetaryen, vegan, glutensiz, fıstık alerjisi...",
    duplicate:
      "Bu isimle daha önce bir cevap gönderilmiş. Yine de göndermek istiyor musun?",
    duplicateConfirm: "Evet, gönder",
    message: "Eklemek istediğiniz bir not",
    submit: "Gönder",
    submitting: "Gönderiliyor...",
    successTitle: "Teşekkür ederiz!",
    successYes: "Cevabınız bize ulaştı. Sizi orada görmek için sabırsızlanıyoruz.",
    successNo: "Cevabınız bize ulaştı. Sizi özleyeceğiz.",
    again: "Yeni bir cevap gönder",
    error: "Bir hata oluştu, lütfen tekrar deneyin.",
    required: "Lütfen adınızı yazın.",
    offline: "İnternet bağlantın yok gibi görünüyor. Bağlanınca tekrar dene.",
    retry: "Tekrar dene",
    closedNow: "Katılım formu az önce kapatıldı. Cevabın kaydedilemedi.",
    closedTitle: "Katılım Formu Kapandı",
    closedDefault:
      "Katılım bildirimi süresi doldu. Yine de bize ulaşmak isterseniz aşağıya bir dilek bırakabilirsiniz.",
    deadlineNote: "Son katılım bildirimi:",
  },
  wishes: {
    eyebrow: "Bize Katılın",
    title: "Dilek Bırakın",
    subtitle: "Bize bir mesaj bırakın, hepsini tek tek okuyacağız.",
    name: "Adınız",
    message: "Dileğinizi buraya yazın...",
    submit: "Gönder",
    submitting: "Gönderiliyor...",
    success: "Dileğiniz için teşekkürler ♥",
    error: "Bir hata oluştu, lütfen tekrar deneyin.",
    required: "Lütfen adınızı ve mesajınızı yazın.",
    empty: "İlk dileği siz bırakın.",
    count: "{n} dilek",
    loadMore: "Daha fazla göster",
    likes: "beğeni",
    like: "Beğen",
    liked: "Beğendiniz",
    topTitle: "En Çok Beğenilen Dilekler",
    seeAll: "Tüm dilekleri gör",
    leaveOne: "Dilek bırak",
    pageSubtitle: "Sevdiklerimizin bize yazdıkları.",
    back: "Ana sayfaya dön",
    sortNew: "En yeni",
    sortTop: "En beğenilen",
    noneYet: "Henüz dilek yok. İlk yazan siz olun.",
    offline: "İnternet bağlantın yok gibi görünüyor. Bağlanınca tekrar dene.",
    retry: "Tekrar dene",
  },
  moments: {
    ctaEyebrow: "Siz de Katılın",
    ctaTitle: "Anı Duvarı",
    ctaText:
      "Çektiğiniz fotoğrafları buraya yükleyin, herkes birbirinin karesini görsün. Tek bir ortak albüm.",
    ctaButton: "Fotoğraflara Git",
    pageTitle: "Anı Duvarı",
    pageSubtitle: "Misafirlerimizin objektifinden düğünümüz.",
    back: "Ana sayfaya dön",
    upload: "Fotoğraf Yükle",
    uploading: "Yükleniyor...",
    uploadTitle: "Anını paylaş",
    yourName: "Adınız",
    caption: "Bir şeyler yaz (isteğe bağlı)",
    choosePhoto: "Fotoğraf seç",
    chooseAnother: "Başka bir fotoğraf seç",
    share: "Paylaş",
    cancel: "Vazgeç",
    empty: "Henüz fotoğraf yok. İlk anıyı siz paylaşın!",
    error: "Yükleme başarısız oldu, lütfen tekrar deneyin.",
    tooBig: "Dosya çok büyük (en fazla 15 MB).",
    needName: "Lütfen adınızı yazın.",
    needPhoto: "Lütfen bir fotoğraf seçin.",
    likes: "beğeni",
    offline: "İnternet bağlantın yok gibi görünüyor. Bağlanınca tekrar dene.",
    retry: "Tekrar dene",
    uploadsClosed: "Fotoğraf yükleme henüz açılmadı. Düğün günü burada buluşuyoruz!",
    close: "Kapat",
    photoCount: "{n} fotoğraf",
  },
  gift: {
    eyebrow: "Sevginizle Yanımızda Olun",
    title: "Düğün Hediyesi",
    text: "Anlayışınız ve sevginiz bizim için en büyük hediye. Yine de katkıda bulunmak isterseniz, aşağıdaki hesabı kullanabilirsiniz.",
    copy: "IBAN'ı Kopyala",
    copied: "Kopyalandı ✓",
    reveal: "Hesabı Göster",
    hide: "Hesabı Gizle",
  },
  closing: {
    eyebrow: "Teşekkür Ederiz",
    title: "Katılımınız ve Dualarınız İçin",
    text: "Bu mutlu günümüzde yanımızda olmanız bizim için büyük bir onur ve mutluluktur. Sevginizi ve iyi dileklerinizi paylaştığınız için teşekkür ederiz.",
    seeYou: "Mutlu Günümüzde Görüşmek Üzere",
  },
  music: {
    play: "Müziği aç",
    pause: "Müziği kapat",
  },
  footer: {
    thanks: "Bizimle bu özel günü paylaştığınız için teşekkür ederiz.",
  },
  story: {
    eyebrow: "Nasıl Başladı",
    title: "Hikayemiz",
    subtitle: "Bugüne kadar geçen yolun birkaç durağı.",
    readAll: "Hikayenin tamamını oku",
    back: "Ana sayfaya dön",
    empty: "Hikayemiz yakında burada olacak.",
  },
  faq: {
    eyebrow: "Merak Ettikleriniz",
    title: "Sıkça Sorulan Sorular",
    subtitle: "Aklınıza takılan bir şey varsa cevabı burada olabilir.",
    empty: "Henüz soru eklenmedi.",
  },
  error: {
    title: "Bir Şeyler Ters Gitti",
    text: "Beklenmedik bir hata oluştu. Sayfayı yenilemeyi deneyin; sorun sürerse birkaç dakika sonra tekrar bakın.",
    retry: "Tekrar dene",
    home: "Ana sayfaya dön",
  },
  notFound: {
    title: "Sayfa Bulunamadı",
    text: "Aradığınız sayfa taşınmış veya hiç var olmamış olabilir.",
    home: "Ana sayfaya dön",
  },
  common: {
    language: "Dil",
  },
};

export type Dict = typeof tr;

const en: Dict = {
  nav: {
    home: "Home",
    couple: "Couple",
    event: "Details",
    gallery: "Gallery",
    rsvp: "RSVP",
    wishes: "Wishes",
    moments: "Moments",
    story: "Our Story",
    faq: "FAQ",
    gift: "Gift",
  },
  cover: {
    eyebrow: "You Are Invited",
    scroll: "Scroll Down",
    theWeddingOf: "The Wedding Of",
    dear: "Dear",
    guest: "Our Honoured Guest",
    nameNote: "Our apologies for any mistake in the spelling of your name or title.",
    open: "Open Invitation",
  },
  couple: {
    eyebrow: "Meet Us",
    title: "The Happy Couple",
    bride: "The Bride",
    groom: "The Groom",
    photoPlaceholder: "Photo",
  },
  countdown: {
    eyebrow: "Time Remaining",
    title: "Counting Down to the Big Day",
    days: "Days",
    hours: "Hours",
    minutes: "Minutes",
    seconds: "Seconds",
    saveDate: "Save The Date",
    addToCalendar: "Add to Calendar",
  },
  events: {
    eyebrow: "When & Where",
    title: "Event Details",
    ceremony: "The Ceremony",
    reception: "The Reception",
    combined: "Ceremony & Reception",
    map: "View on Map",
  },
  gallery: {
    eyebrow: "Our Memories",
    title: "Photo Gallery",
    empty: "Photos coming soon.",
  },
  video: {
    eyebrow: "Our Story",
    title: "Video",
  },
  rsvp: {
    eyebrow: "Please Let Us Know",
    title: "RSVP",
    subtitle: "Kindly let us know whether you can join us, so we can save your seat.",
    name: "Full name",
    email: "Email (optional)",
    phone: "Phone (optional)",
    attendingQuestion: "Will you be able to join us?",
    yes: "Yes, I'll be there",
    no: "Sorry, I can't make it",
    guestCount: "How many of you are coming?",
    side: "Whose guest are you?",
    sideBride: "Bride",
    sideGroom: "Groom",
    sideBoth: "Both",
    diet: "Dietary preference or allergy (optional)",
    dietHint: "Vegetarian, vegan, gluten-free, nut allergy...",
    duplicate:
      "A response has already been sent under this name. Would you like to send another?",
    duplicateConfirm: "Yes, send it",
    message: "Anything you'd like to add",
    submit: "Send",
    submitting: "Sending...",
    successTitle: "Thank you!",
    successYes: "We've received your answer. We can't wait to see you there.",
    successNo: "We've received your answer. You will be missed.",
    again: "Send another response",
    error: "Something went wrong, please try again.",
    required: "Please enter your name.",
    offline: "You appear to be offline. Please try again once you reconnect.",
    retry: "Try again",
    closedNow: "The RSVP form has just been closed. Your answer was not saved.",
    closedTitle: "RSVP Is Now Closed",
    closedDefault:
      "The RSVP period has ended. If you would still like to reach us, please leave a wish below.",
    deadlineNote: "Please reply by:",
  },
  wishes: {
    eyebrow: "Join Us",
    title: "Leave a Wish",
    subtitle: "Write us a message — we will read every single one.",
    name: "Your name",
    message: "Write your wish here...",
    submit: "Send",
    submitting: "Sending...",
    success: "Thank you for your wish ♥",
    error: "Something went wrong, please try again.",
    required: "Please enter your name and message.",
    empty: "Be the first to leave a wish.",
    count: "{n} wishes",
    loadMore: "Show more",
    likes: "likes",
    like: "Like",
    liked: "Liked",
    topTitle: "Most Loved Wishes",
    seeAll: "See all wishes",
    leaveOne: "Leave a wish",
    pageSubtitle: "Words written to us by the people we love.",
    back: "Back to home",
    sortNew: "Newest",
    sortTop: "Most loved",
    noneYet: "No wishes yet. Be the first to write one.",
    offline: "You appear to be offline. Please try again once you reconnect.",
    retry: "Try again",
  },
  moments: {
    ctaEyebrow: "Join In",
    ctaTitle: "The Memory Wall",
    ctaText:
      "Upload the photos you take and see everyone else's shots too. One shared album for all of us.",
    ctaButton: "Go to Photos",
    pageTitle: "The Memory Wall",
    pageSubtitle: "Our wedding through the eyes of our guests.",
    back: "Back to home",
    upload: "Upload Photo",
    uploading: "Uploading...",
    uploadTitle: "Share your moment",
    yourName: "Your name",
    caption: "Say something (optional)",
    choosePhoto: "Choose a photo",
    chooseAnother: "Choose another photo",
    share: "Share",
    cancel: "Cancel",
    empty: "No photos yet. Be the first to share a moment!",
    error: "Upload failed, please try again.",
    tooBig: "File is too large (15 MB max).",
    needName: "Please enter your name.",
    needPhoto: "Please choose a photo.",
    likes: "likes",
    offline: "You appear to be offline. Please try again once you reconnect.",
    retry: "Try again",
    uploadsClosed: "Photo uploads are not open yet. See you on the wedding day!",
    close: "Close",
    photoCount: "{n} photos",
  },
  gift: {
    eyebrow: "Your Love Is Enough",
    title: "Wedding Gift",
    text: "Your presence and your love are the greatest gift to us. If you would still like to contribute, you may use the account below.",
    copy: "Copy IBAN",
    copied: "Copied ✓",
    reveal: "Show Account",
    hide: "Hide Account",
  },
  closing: {
    eyebrow: "Thank You",
    title: "For Your Presence and Your Blessings",
    text: "It would be our greatest honour and joy to have you beside us on this happy day. Thank you for sharing your love and your kind wishes with us.",
    seeYou: "See You on Our Happy Day",
  },
  music: {
    play: "Play music",
    pause: "Pause music",
  },
  footer: {
    thanks: "Thank you for sharing this special day with us.",
  },
  story: {
    eyebrow: "How It Began",
    title: "Our Story",
    subtitle: "A few of the stops along the way here.",
    readAll: "Read the whole story",
    back: "Back to home",
    empty: "Our story will be here soon.",
  },
  faq: {
    eyebrow: "Good to Know",
    title: "Frequently Asked Questions",
    subtitle: "If something is on your mind, the answer may be here.",
    empty: "No questions have been added yet.",
  },
  error: {
    title: "Something Went Wrong",
    text: "An unexpected error occurred. Try reloading the page; if it persists, please come back in a few minutes.",
    retry: "Try again",
    home: "Back to home",
  },
  notFound: {
    title: "Page Not Found",
    text: "The page you are looking for may have moved, or may never have existed.",
    home: "Back to home",
  },
  common: {
    language: "Language",
  },
};

const it: Dict = {
  nav: {
    home: "Home",
    couple: "Sposi",
    event: "Dettagli",
    gallery: "Galleria",
    rsvp: "Conferma",
    wishes: "Auguri",
    moments: "Ricordi",
    story: "La Nostra Storia",
    faq: "FAQ",
    gift: "Regalo",
  },
  cover: {
    eyebrow: "Siete Invitati",
    scroll: "Scorri Giù",
    theWeddingOf: "Il Matrimonio Di",
    dear: "Gentile",
    guest: "Nostro Caro Ospite",
    nameNote: "Ci scusiamo per eventuali errori nella scrittura del nome o del titolo.",
    open: "Apri l'Invito",
  },
  couple: {
    eyebrow: "Chi Siamo",
    title: "Gli Sposi",
    bride: "La Sposa",
    groom: "Lo Sposo",
    photoPlaceholder: "Foto",
  },
  countdown: {
    eyebrow: "Tempo Rimanente",
    title: "Conto Alla Rovescia",
    days: "Giorni",
    hours: "Ore",
    minutes: "Minuti",
    seconds: "Secondi",
    saveDate: "Salva la Data",
    addToCalendar: "Aggiungi al Calendario",
  },
  events: {
    eyebrow: "Quando e Dove",
    title: "Dettagli dell'Evento",
    ceremony: "La Cerimonia",
    reception: "Il Ricevimento",
    combined: "Cerimonia e Ricevimento",
    map: "Vedi sulla Mappa",
  },
  gallery: {
    eyebrow: "I Nostri Ricordi",
    title: "Galleria Fotografica",
    empty: "Le foto arriveranno presto.",
  },
  video: {
    eyebrow: "La Nostra Storia",
    title: "Video",
  },
  rsvp: {
    eyebrow: "Fatecelo Sapere",
    title: "Conferma di Partecipazione",
    subtitle:
      "Vi chiediamo gentilmente di confermare la vostra presenza, così da riservarvi il posto.",
    name: "Nome e cognome",
    email: "Email (facoltativo)",
    phone: "Telefono (facoltativo)",
    attendingQuestion: "Riuscirete a partecipare?",
    yes: "Sì, ci sarò",
    no: "Purtroppo non posso",
    guestCount: "In quanti verrete?",
    side: "Di chi siete ospiti?",
    sideBride: "Sposa",
    sideGroom: "Sposo",
    sideBoth: "Entrambi",
    diet: "Preferenze alimentari o allergie (facoltativo)",
    dietHint: "Vegetariano, vegano, senza glutine, allergia alle noci...",
    duplicate:
      "Una risposta con questo nome è già stata inviata. Volete inviarne un'altra?",
    duplicateConfirm: "Sì, invia",
    message: "Qualcosa che volete aggiungere",
    submit: "Invia",
    submitting: "Invio in corso...",
    successTitle: "Grazie di cuore!",
    successYes: "Abbiamo ricevuto la vostra risposta. Non vediamo l'ora di vedervi.",
    successNo: "Abbiamo ricevuto la vostra risposta. Ci mancherete.",
    again: "Invia un'altra risposta",
    error: "Si è verificato un errore, riprovate.",
    required: "Inserite il vostro nome.",
    offline: "Sembra che tu sia offline. Riprova quando torni online.",
    retry: "Riprova",
    closedNow: "Il modulo è stato appena chiuso. La tua risposta non è stata salvata.",
    closedTitle: "Le Conferme Sono Chiuse",
    closedDefault:
      "Il periodo per confermare la presenza è terminato. Se desiderate comunque scriverci, lasciate un augurio qui sotto.",
    deadlineNote: "Rispondete entro:",
  },
  wishes: {
    eyebrow: "Unitevi a Noi",
    title: "Lasciate un Augurio",
    subtitle: "Scriveteci un messaggio: li leggeremo tutti, uno per uno.",
    name: "Il vostro nome",
    message: "Scrivete qui il vostro augurio...",
    submit: "Invia",
    submitting: "Invio in corso...",
    success: "Grazie per il vostro augurio ♥",
    error: "Si è verificato un errore, riprovate.",
    required: "Inserite nome e messaggio.",
    empty: "Siate i primi a lasciare un augurio.",
    count: "{n} auguri",
    loadMore: "Mostra altri",
    likes: "mi piace",
    like: "Mi piace",
    liked: "Ti piace",
    topTitle: "Gli Auguri Più Amati",
    seeAll: "Vedi tutti gli auguri",
    leaveOne: "Lascia un augurio",
    pageSubtitle: "Le parole che ci hanno scritto le persone che amiamo.",
    back: "Torna alla home",
    sortNew: "Più recenti",
    sortTop: "Più amati",
    noneYet: "Ancora nessun augurio. Scrivete il primo.",
    offline: "Sembra che tu sia offline. Riprova quando torni online.",
    retry: "Riprova",
  },
  moments: {
    ctaEyebrow: "Partecipate Anche Voi",
    ctaTitle: "Il Muro dei Ricordi",
    ctaText:
      "Caricate le foto che scattate e guardate quelle degli altri. Un unico album condiviso.",
    ctaButton: "Vai alle Foto",
    pageTitle: "Il Muro dei Ricordi",
    pageSubtitle: "Il nostro matrimonio visto dai nostri ospiti.",
    back: "Torna alla home",
    upload: "Carica Foto",
    uploading: "Caricamento...",
    uploadTitle: "Condividi il tuo ricordo",
    yourName: "Il vostro nome",
    caption: "Scrivete qualcosa (facoltativo)",
    choosePhoto: "Scegli una foto",
    chooseAnother: "Scegli un'altra foto",
    share: "Condividi",
    cancel: "Annulla",
    empty: "Ancora nessuna foto. Condividete il primo ricordo!",
    error: "Caricamento non riuscito, riprovate.",
    tooBig: "File troppo grande (max 15 MB).",
    needName: "Inserite il vostro nome.",
    needPhoto: "Scegliete una foto.",
    likes: "mi piace",
    offline: "Sembra che tu sia offline. Riprova quando torni online.",
    retry: "Riprova",
    uploadsClosed: "Il caricamento delle foto non è ancora aperto. Ci vediamo il giorno del matrimonio!",
    close: "Chiudi",
    photoCount: "{n} foto",
  },
  gift: {
    eyebrow: "Il Vostro Affetto Ci Basta",
    title: "Regalo di Nozze",
    text: "La vostra presenza e il vostro affetto sono per noi il regalo più grande. Se desiderate comunque contribuire, potete usare il conto qui sotto.",
    copy: "Copia IBAN",
    copied: "Copiato ✓",
    reveal: "Mostra il Conto",
    hide: "Nascondi il Conto",
  },
  closing: {
    eyebrow: "Grazie di Cuore",
    title: "Per la Vostra Presenza e i Vostri Auguri",
    text: "Sarebbe per noi un grande onore e una grande gioia avervi accanto in questo giorno felice. Grazie per aver condiviso con noi il vostro affetto e i vostri auguri.",
    seeYou: "Ci Vediamo nel Nostro Giorno Felice",
  },
  music: {
    play: "Riproduci la musica",
    pause: "Metti in pausa",
  },
  footer: {
    thanks: "Grazie per aver condiviso con noi questo giorno speciale.",
  },
  story: {
    eyebrow: "Come È Cominciata",
    title: "La Nostra Storia",
    subtitle: "Alcune tappe del cammino che ci ha portati fin qui.",
    readAll: "Leggi tutta la storia",
    back: "Torna alla home",
    empty: "La nostra storia arriverà presto.",
  },
  faq: {
    eyebrow: "Buono a Sapersi",
    title: "Domande Frequenti",
    subtitle: "Se avete qualche dubbio, la risposta potrebbe essere qui.",
    empty: "Non è ancora stata aggiunta nessuna domanda.",
  },
  error: {
    title: "Qualcosa È Andato Storto",
    text: "Si è verificato un errore imprevisto. Provate a ricaricare la pagina; se il problema persiste, riprovate tra qualche minuto.",
    retry: "Riprova",
    home: "Torna alla home",
  },
  notFound: {
    title: "Pagina Non Trovata",
    text: "La pagina che cercate potrebbe essere stata spostata o non essere mai esistita.",
    home: "Torna alla home",
  },
  common: {
    language: "Lingua",
  },
};

const dictionaries: Record<Locale, Dict> = { tr, en, it };

export function getDictionary(locale: Locale): Dict {
  return dictionaries[locale] ?? dictionaries.tr;
}

/** "{n} dilek" gibi metinlerdeki sayıyı yerine koyar. */
export function withCount(template: string, n: number) {
  return template.replace("{n}", String(n));
}
